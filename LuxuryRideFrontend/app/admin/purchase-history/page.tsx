...
import { Calendar, Car, DollarSign, Search, User } from "lucide-react";
...
export default function AdminPurchaseHistoryPage() {
    ...
    async function load() {
      try {
        ...
        const purchases: TransactionDTO[] = purchasesRaw.map((p: any) => ({
          id:            p.id + 100000,          // prevent clash with rental IDs
          type:          "purchase",
  ...
  const activeRentals = visible.filter(t => t.type === "rental" && t.status === "pending").length;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        <h1 className="text-3xl font-bold mb-2">Admin History</h1>
        <p className="text-gray-600 mb-6">Manage all rentals and sales</p>
...
function StatCard({ title, icon: Icon, value }:{
  title: string; icon: any; value: any;
}) {
  return (
    <Card>
...
function TransactionList({ transactions }:{ transactions: TransactionDTO[] }) {
  return (
    <div className="space-y-4">
      {transactions.map(t => (
        <Card key={`${t.type}-${t.id}`} className="hover:shadow-md">
          <CardHeader>
...
function Field({ icon: Icon, label, sub }:{
